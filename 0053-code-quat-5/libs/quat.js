...
function quat() {
  ...
  // Episode 31
  this.log = function(theta) {
    var alpha = Math.acos(this.w);
    var sina = Math.sin(alpha);
    var q = new quat().create();
    q.w = 0;
    q.x = this.x * alpha / sina;
    q.y = this.y * alpha / sina;
    q.z = this.z * alpha / sina;
    return q;
  };

  this.exp = function(theta) {
    var alpha = this.magnitude();
    var sina = Math.sin(alpha);
    var p = new quat().create();
    p.w = Math.cos(alpha);
    p.x = this.x * sina / alpha;
    p.y = this.y * sina / alpha;
    p.z = this.z * sina / alpha;
    return p;
  };
